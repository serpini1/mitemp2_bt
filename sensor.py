...
def setup_platform(hass, config, add_entities, discovery_info=None):
    """Set up the MiTemp2Bt sensor."""
    backend = BACKEND
    _LOGGER.debug("MiTemp2Bt is using %s backend.", backend.__name__)

    cache = config.get(CONF_CACHE)
    poller = mitemp2_bt_poller.MiTemp2BtPoller(
        config.get(CONF_MAC),
        cache_timeout=cache,
    ...
    for parameter in config[CONF_MONITORED_CONDITIONS]:
        ...
        devs.append(
            MiTemp2BtSensor(poller, parameter, device, name, unit, force_update, median)
        )

    add_entities(devs)


class MiTemp2BtSensor(Entity):
    """Implementing the MiTemp2Bt sensor."""
